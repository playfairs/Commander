...
final class MainViewController: NSViewController {
    ...
    @objc func goToPath(_ sender: Any?) {
        ...
        presentTextPopup(title: "Go to Folder", subtitle: prompt, placeholder: "/path/to/folder", acceptTitle: "Open") { [weak self] path in
    ...
    @objc func quickSearch(_ sender: Any?) {
        promptForText(title: "Quick Search", message: "Search the current folder and its parents.", placeholder: "Search term") { [weak self] query in
    ...
    @objc func fullSearch(_ sender: Any?) {
        promptForText(title: "Full Search", message: "Search all mounted volumes and your home folder.", placeholder: "Search term") { [weak self] query in
    ...
    private func promptForText(title: String, message: String, placeholder: String, completion: @escaping (String) -> Void) {
        presentTextPopup(title: title, subtitle: message, placeholder: placeholder, acceptTitle: "OK", completion: completion)
    }
    ...
    private func presentTextPopup(title: String, subtitle: String, placeholder: String, acceptTitle: String, completion: @escaping (String) -> Void) {
        removePopupOverlay()
        ...
        let cancelButton = NSButton(title: "Cancel", target: self, action: #selector(popupCancelAction(_:)))
        cancelButton.bezelStyle = .rounded
        let okButton = NSButton(title: acceptTitle, target: self, action: #selector(popupConfirmAction(_:)))
...
private final class BrowserPaneViewController: NSViewController {
    ...
    private let backButton = NSButton(title: "", image: NSImage(systemSymbolName: "chevron.left", accessibilityDescription: "Back") ?? NSImage(), target: nil, action: nil)
    private let forwardButton = NSButton(title: "", image: NSImage(systemSymbolName: "chevron.right", accessibilityDescription: "Forward") ?? NSImage(), target: nil, action: nil)
    private let upButton = NSButton(title: "", image: NSImage(systemSymbolName: "arrow.up", accessibilityDescription: "Up") ?? NSImage(), target: nil, action: nil)
    ...
    fileprivate func loadDirectory(_ url: URL, replaceHistory: Bool) {
        ...
        let rawItems = try FileManager.default.contentsOfDirectory(at: url, includingPropertiesForKeys: [.isDirectoryKey, .contentTypeKey, .fileSizeKey, .contentModificationDateKey], options: options)
    ...
    func tableView(_ tableView: NSTableView, acceptDrop info: NSDraggingInfo, row: Int, dropOperation: NSTableView.DropOperation) -> Bool {
        let options: [NSPasteboard.ReadingOptionKey: Any] = [.urlReadingFileURLsOnly: true]
        guard let urls = info.draggingPasteboard.readObjects(forClasses: [NSURL.self], options: options) as? [URL], !urls.isEmpty else {
            return false
        }
    ...
    private func promptForDropAction(urls: [URL]) -> (DropAction, [String]?)? {
        ...
        alert.informativeText = urls.count == 1 ? "Choose whether to copy or move the file into this folder, and provide a new name." : "Choose whether to copy or move these files into this folder. Names will remain unchanged."
        alert.alertStyle = .informational
        ...
        case .alertFirstButtonReturn:
            let name = urls.count == 1 ? nameField?.stringValue.trimmingCharacters(in: .whitespacesAndNewlines) : nil
    ...
    private func performDrop(urls: [URL], action: DropAction, names: [String]?) throws {
        for (index, sourceURL) in urls.enumerated() {
            let destinationName = names?.indices.contains(index) == true ? names![index] : sourceURL.lastPathComponent
            let destinationURL = uniqueDestinationURL(for: currentURL.appendingPathComponent(destinationName))
            switch action {
            case .copy:
    ...
    private func uniqueDestinationURL(for url: URL) -> URL {
        ...
        while FileManager.default.fileExists(atPath: destination.path) {
            let candidate = extensionPart.isEmpty ? "\(originalName) \(suffix)" : "\(originalName) \(suffix).\(extensionPart)"
    ...
    private func reloadSiblingPanesAfterMove(from sourceFolders: [URL]) {
        ...
        view.window?.contentViewController?.children.forEach { controller in
            if let pane = controller as? BrowserPaneViewController, pane !== self, uniqueSources.contains(pane.currentURL) {
                pane.reloadCurrentDirectory()
            }
        }
    }
    ...
    fileprivate func contextMenuForSelectedItem() -> NSMenu? {
        ...
        let quickLookItem = NSMenuItem(title: "Quick Look \(item.name)", action: #selector(performQuickLookAction), keyEquivalent: "")
        quickLookItem.target = self
        menu.addItem(quickLookItem)
        let renameItem = NSMenuItem(title: "Rename", action: #selector(renameSelectedItem), keyEquivalent: "")
        renameItem.target = self
        menu.addItem(renameItem)
        menu.addItem(NSMenuItem.separator())
        let showInFinderItem = NSMenuItem(title: "Show in Finder", action: #selector(revealSelectedItemInFinder), keyEquivalent: "")
        showInFinderItem.target = self
        menu.addItem(showInFinderItem)
        let trashItem = NSMenuItem(title: "Move to Trash", action: #selector(performTrashAction), keyEquivalent: "")
        trashItem.target = self
        menu.addItem(trashItem)
        let deleteItem = NSMenuItem(title: "Delete Immediately", action: #selector(performPermanentDeleteAction), keyEquivalent: "")
        deleteItem.target = self
        menu.addItem(deleteItem)
        menu.addItem(NSMenuItem.separator())
        let infoItem = NSMenuItem(title: "Get Info", action: #selector(showSelectedInfo), keyEquivalent: "")
...
private struct DirectoryItem: Comparable {
    ...
    init(url: URL, isParent: Bool = false) {
        ...
        if isParent {
            ...
            let resourceValues = try? url.resourceValues(forKeys: [.isDirectoryKey, .contentTypeKey, .fileSizeKey, .contentModificationDateKey, .isHiddenKey])
        isDirectory = resourceValues?.isDirectory ?? false
        isHidden = resourceValues?.isHidden ?? url.lastPathComponent.hasPrefix(".")
        kind = isDirectory ? "Folder" : (url.pathExtension.uppercased().isEmpty ? "File" : url.pathExtension.uppercased())
...
private extension BrowserPaneViewController {
    static var defaultDesktopURL: URL {
